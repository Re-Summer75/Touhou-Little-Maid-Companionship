...
/**
 * Classifies common model-part aliases without depending on a model library.
 */
public final class FaceBoneClassifier {
    ...
    public static int anchorPriority(String name) {
        NameParts parts = parse(name);
        if (parts.excluded()
                || parts.compact().endsWith("locator")) {
            return -1;
        }
        return switch (parts.compact()) {
            ...
            default -> {
                ...
                yield parts.tokens().contains("head")
                        ? 65
                        : -1;
            }
        };
    ...
    public static Role classify(String name, Role inheritedRole) {
        ...
        if (parts.tokens().stream().anyMatch(
                FEATURE_TOKENS::contains
        )) {
            return Role.FEATURE;
        }
    ...
    private static NameParts parse(String name) {
        ...
        boolean excluded = tokens.stream().anyMatch(
                EXCLUDED_TOKENS::contains
