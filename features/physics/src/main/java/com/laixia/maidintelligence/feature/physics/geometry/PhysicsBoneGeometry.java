...
public final class PhysicsBoneGeometry {
    ...
    public static Analysis analyze(BoneModelSnapshot model) {
        IdentityHashMap<BoneModelSnapshot.Bone, MutableNode> mutable = new IdentityHashMap<>();
        ...
        IdentityHashMap<BoneModelSnapshot.Bone, Node> nodes = new IdentityHashMap<>();
    ...
    private static void collect(
        ...
        List<CubeBox> cubeBoxes = measureCubes(bone.geometry().cubes(), transform);
     ...
     */
    private static List<CubeBox> measureCubes(BoneModelSnapshot.Mesh mesh, Matrix4f transform) {
    ...
    private static Bounds nearestSolidDescendantBounds(
            Node root,
            IdentityHashMap<BoneModelSnapshot.Bone, Node> nodes
    ) {
    ...
    public record Node(
        ...
        public boolean hasGeometry() {
            return bone.geometry().cubes().getCubeCount() > 0 && !bounds.isEmpty();
        }
    ...
    public static final class Bounds {
        private static final Bounds EMPTY = new Bounds(null);
        ...
        public Bounds union(Bounds other) {
            ...
            return isEmpty()
                    ? other
                    : new Bounds(value.union(other.value));
        }
