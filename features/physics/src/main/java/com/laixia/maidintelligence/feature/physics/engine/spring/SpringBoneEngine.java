...
public final class SpringBoneEngine {
    ...
    public float projectionDamping(int drivenSlot) {
        return context.state.oscillation.projectionDamping[drivenSlot];
    }

    public int projectionReversals(int drivenSlot) {
        return context.state.oscillation.projectionReversals[drivenSlot];
    }

    /** How much of a resting surface one segment is currently relying on. */
    public float contactSupport(int drivenSlot) {
        return context.state.contact.support[drivenSlot];
    }

    public float lastIntegratorStep(int drivenSlot) {
        return context.state.integration.lastIntegratorStep[drivenSlot];
    }

    /** Bit 1 for a swing limit, bit 2 for a collider. */
    public int lastProjectionSource(int drivenSlot) {
        return context.state.integration.lastProjectionSource[drivenSlot];
    }
    ...
    public boolean copyRestDirection(int drivenSlot, Vector3f output) {
        if (drivenSlot < 0
                || drivenSlot
                >= context.state.integration.restDirections.length) {
            return false;
        }
        Vector3f rest =
                context.state.integration.restDirections[drivenSlot];
    ...
    public float lastProjectionStep(int drivenSlot) {
        return context.state.integration.lastProjectionStep[drivenSlot];
    }
