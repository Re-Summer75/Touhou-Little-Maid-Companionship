...
/**
 * 根进度图标条的换算。女仆界面内容区较窄，因此使用「左右箭头 + 一页若干图标」。
 */
public final class AdvancementStripLayout {
    ...
    public static int perPage(int stripWidth) {
        return Math.max(
                1,
                (stripWidth - 2 * ARROW_WIDTH) / SLOT_SIZE
        );
    }
    ...
    public static int slotAt(
        ...
        if (slot >= perPage
                || offset - slot * SLOT_SIZE >= ICON_SIZE) {
            return NO_SLOT;
        }
        return slot;
    }
    ...
    public static boolean inPreviousArrow(
            int stripLeft,
            int stripTop,
            int mouseX,
            int mouseY
    ...
    public static boolean inNextArrow(
            int stripLeft,
            int stripWidth,
            int stripTop,
            int mouseX,
        ...
        return inArrow(
                nextArrowX(stripLeft, stripWidth),
                stripTop,
                mouseX,
                mouseY
        );
    ...
    private static boolean inArrow(
            int arrowX,
            int stripTop,
            int mouseX,
            int mouseY
