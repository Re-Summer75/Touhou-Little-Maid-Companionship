...
import com.laixia.maidintelligence.feature.atmosphere.domain.AdvectedGradientNoise;
...
/**
 * Deterministic, allocation-free environmental wind math.
 */
public final class EnvironmentalWindField {
     ...
     */
    public static WindVector targetInto(
            double worldX,
            double worldZ,
        ...
        return targetInto(
                ...
                strength,
                output,
                DefaultWindVectorPort.INSTANCE
        );
    }

    public static <V> V targetInto(
            double worldX,
            double worldZ,
            double worldTimeTicks,
            int dimensionHash,
            float strength,
            V output,
            MutableWindVectorPort<V> vectors
    ) {
        return targetInto(
                worldX,
                worldZ,
    ...
    public static <V> V targetInto(
        ...
        float forward = safeStrength * forwardGust;
    ...
    public static WindVector smoothInto(
            WindVector current,
            WindVector target,
            float dt,
            WindVector output
    ...
    public static <V> V smoothInto(
        ...
        if (!Float.isFinite(dt) || dt <= 0.0F) {
            return vectors.copy(output, current);
        }
        ...
        vectors.copy(output, current);
        return vectors.lerp(output, target, alpha);
    }
