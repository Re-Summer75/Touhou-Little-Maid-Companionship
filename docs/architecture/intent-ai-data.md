...
内置目录不使用它（一律为 `1.0`）。**优先级只表达"谁能打断谁"，不表达偏好** ——
一个意图独占一个优先级时，它的分数永远不会被拿来和任何东西比较，效用层随之失效。
内置意图因此收敛为五个 band：安全 `100`、主人命令 `80`、生存 `50`、陪伴 `30`、自娱 `10`。
新增意图应归入既有 band 并用分数表达偏好；`verifyCompanionPriorityBand` 会拒绝新的
优先级数字、拒绝 `SUM` 聚合、拒绝低于 1.0 的 `activation_chance`，并要求每个意图
在其考量的最优取值下能超过自身 `minimum_score`。

两条容易踩的性质：

- `product` 下 `factor = 1 - weight × (1 - curved)`。单项 `weight` 为 `1.0` 时，事实处于
  区间下限会让分数归零。这是刻意保留的否决能力，但用在主行为上会让它永远赢不了。
- `minimum_score` 为 `0` 时零分候选**不会**被丢弃（比较是 `score < minimum_score`）。
  依赖否决的意图须把它设到 `0` 以上。
