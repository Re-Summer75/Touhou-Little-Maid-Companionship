...
作者姿态优先于任何物理效果：静止不动时，每根驱动骨都精确停在模型做出来的位置，物理只在这个位置周围添加运动。重力、风和碰撞宽限三条路径都曾各自把静止造型搬走，现在都以作者姿态为参照量。
...
- 每帧用 **Verlet 积分**更新:
  ...
  3. **外力**：`(相对姿态的重力 + 实体移动惯性 + 预设动画惯性 + 转身伪力) × dt`；
  4. 归一化到单位长度。

因为刚度目标是**动画姿态**而非世界下方、重力也以该姿态为参照，所以:

- **静止零位移**：作者画裙摆下垂、腰带侧躺时，画的已经是重力作用后的形态，再叠加一次世界下方的恒力就等于把重力算了两遍——恒力作用在弹簧上必然把它停在"力与恢复力平衡"处而不是画出来的位置，上翘的发丝会垮下来、横向的丝带会歪掉，且调小强度只能减轻不能消除。因此重力按静止方向分解：**平行分量**无条件生效，方向随后被归一化，所以它只改变"被拉回去的力度"而不改变"回到哪里"，下垂部件因此更沉、逆重力上举的部件更软；**垂直分量**才是会搬动造型的那一半，按当前已偏离静止方向的程度淡入。于是静止时精确保持作者姿态，被风或被腿踢开之后重力仍朝真实的下方把它拽落。该分量在满偏离时饱和于完整重力，而恢复力随角度继续增长，因此不会自激；
...
- **重力**：以 `GRAVITY_POWER` 为大小，但沿静止方向分解后注入：平行分量常驻、垂直分量按偏离程度淡入，静止姿态因此不被搬动（见上文核心原则）。悬垂部件使用分类型倍率，`HEAD_SHELL` 仅保留近乎为零的重力，避免抬头时整块头盖向后翻;
...
重力改为相对姿态求解本身几乎不花钱（每段一个点积）。同一次会话内与旧的世界重力做过对照：`298 µs` 对 `324 µs`，方向与直觉相反且落在噪声里——同一份代码在不同会话测出过 `298` 和 `350 µs`，因此这类个位数百分比的差异只能同会话对照，不能跨会话比较。
