...
- 显式代理的位置和形状半径都使用 **Gecko / Bedrock 模型空间像素**，`16 px = 1` 个模型空间方块；`normal` 只是无单位方向。可选 `hit_radius` 也是像素，表示受驱动骨末端的碰撞半径；省略时对刚性网格取 `0.1 px` 的数值容差（以碰撞体最薄半轴的 `15%` 为上限），布料层取外层布片自身半厚，最终仍会乘 `hit_radius_scale`。
...
- 刚性网格的末端半径则是纯数值容差（`0.1 px`，上限为碰撞体最薄半轴的 `15%`），因为末端是骨轴上的点、网格相对该轴的位置未知，测量骨骼无法回答"轴停在离表面多远才算贴合"。此前按受驱动骨最薄边的一半推导，实测每一对都由那个 `15%` 上限胜出，末端半径于是只与碰撞体最薄半轴成正比而与布片厚度无关：同一片裙子贴躯干被顶开 `0.675 px`、贴 `0.3 px` 薄饰片只有 `0.045 px`，越厚的碰撞体把布料悬浮得越远；
