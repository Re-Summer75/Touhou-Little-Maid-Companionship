...
> 状态：阶段 1～13 已落地；裙摆 XPBD 与高速碰撞稳定化均已启用。
>
...
对 Create Aeronautics / Simulated 的源码对照也只迁移适合骨段约束模型的
原则：其核心碰撞实际由 Sable + Rapier 承担，刚体使用固定子步、迭代接触、
体素表面过滤和缓存；实体碰撞则按相对行程插值有限个姿态。这里不引入 native
Rapier，也不把全模型时间步翻倍，而是采用加载期内部面过滤、上一帧活跃候选
连贯、以及仅对“两端清空但路径穿越”的单对 Box/Sphere 做 `2～4` 个选择性
子步。Sable 的自定义体素碰撞本身也不具备完整 shape cast，不能把“启用 CCD”
误写成所有碰撞体都连续。
...
5. 将历史方向搬运到本链的参考空间；
6. 执行 Verlet 惯性、刚度、实体运动和动画运动积分；生产约束路径不追加恒定重力扭矩；
7. 动态输入消失且可见偏移超过 `2 px` 时，按 `96 px/s` 限制朝作者姿态的单帧回程；
8. 投影动画姿态相对的非对称摆角约束；
9. 按顺序投影当前骨骼关联的 Box / Plane / Sphere / Capsule 代理；
10. 最多交替复查摆角和碰撞四轮；代理内部也按固定上限复查多代理；
11. 对同一刚性安装座下的裙摆链根投影 XPBD 最大距离锁链，再复查全局安全摆角和碰撞；
12. 对被无解接触抑制但仍被风压入的代理，沿作者方向搜索最近合法边界；
13. 检测碰撞连续取消回程的路径锁，并向缓存的近作者合法姿态恢复；
14. 将最终合法方向写回当前和历史状态；
15. 根据合法方向生成骨骼旋转和虚拟枢轴位移补偿。
...
`rotationInertiaScale` 压到 `0.10` 以下。所有生产约束链都以作者姿态
作为零静态垂量平衡点；`HEAD_SHELL` 与自动悬垂饰品再通过高刚度/阻尼、
低旋转惯性和小摆角保持刚性观感。

### 作者姿态与静态重力

模型作者画出的裙摆、发丝和挂饰已经是受重力后的成品姿态。若解算器继续按
`(0, -g, 0)` 恒定注入，弹簧必然停在“重力与恢复力平衡”的第二个位置：
上翘发丝会塌、横向丝带会歪，调小 `gravityScale` 只能减轻而不能消除。
`winefox` 的实测最坏静态可见偏移达到 `44.660 px`。

当前生产约束路径因此不再施加恒定重力扭矩，作者姿态成为唯一静态平衡点；
同一夹具静止三秒后的最坏偏移降到 `0.048 px`，残量来自碰撞数值容差。
重量感仍由四类动态信号组成：
...
`gravityScale` 仍被 schema 接受，以兼容既有 sidecar 和无约束 reference
基准通道；约束生产路径不读取它来制造静态偏转。该取舍优先保留作者轮廓，
而不是在已经垂下的网格上重复模拟一次世界重力。
...
无受驱动祖先且通过碰撞源资格门的静止 cube 成为一个 Box 代理。cube 自带 `dx/dy/dz`
边框，所以 Box 与网格严格等同并保留 cube 自身旋转；若按骨骼取 AABB，
散布着几十个 cube 的发套、法阵或披挂会被膨胀成大半是空气的包围盒，
既挡住空处又看不出对应几何。头、上身、手臂、腿都不再需要作者摆放胶囊。

刚性只表示“不由弹簧驱动”，不自动获得碰撞资格。统一的装备分类器与
`AutomaticCollisionSourcePolicy` 会排除手持物完整结构分支、命名武器、
locator、帽具/耳机、战术穿戴物、表情贴片、渲染特效和 `dontRender` 网格；
裙摆刚性安装座等结构表面继续保留。手持分支按 TLM locator 拓扑识别，
因此 `sta` 的匿名 `74m/74m3` 无需词典也不会把枪械零件烘焙成代理。

- `HAIR`、`EAR`、`HEAD_LOCAL RIBBON`：头、上身及可达手臂的网格 Box；
- `SKIRT`、`BODY_LOCAL RIBBON`：躯干与左右腿的网格 Box；
- `CAPE`：网格 Box + 随 Body 运动的 Back Plane。

附着数量对**通过资格门的结构表面**不设上限：一个段够得到的合格 cube
全部保留，碰撞面因此与模型一一对应，而不是靠“挑几个代表”近似。随后做：
...
`winefox` 由此得到 `3229` 个代理覆盖 `80` 个受驱动段，远处的灯笼、武器
仍不进入热路径。数量放开后的成本由共享形状、逐帧锥剪枝、组内空间细分、
Top-K 求解、延后绑定和间隙留存六层结构控制（见性能门槛）。若某模型完全
没有可用刚性网格，
才回退到旧的拟合 Body Capsule。

运行时另同步 Gecko 的 `isHidden`、`cubesAreHidden` 与祖先隐藏状态。
不可见参考的自动网格/层代理不参与校准、排序、投影和调试绘制；重新显示
时才以当帧作者姿态惰性校准，避免备用装备在原位置留下“幽灵碰撞体”。
显式 schema 3 代理是作者声明的物理边界，故不受该渲染门控。
...
- 两块 cube 都是**薄板**——最薄轴不超过次薄轴的 `0.5`，浮点变换后的边界
  允许一个比率 ULP，保证作者给出的精确 `1:2` 薄板不被舍入误杀；蝴蝶结、
  绳扣一类方块仍然出局；
...
碰撞检测使用每段骨骼末端球，而不是无半径点，并可由 schema 3 元数据覆盖；
投影始终维持固定骨长。刚性网格路径的 `hitRadius` 是**数值容差**，不是厚度：
末端是骨轴上的一个点，而网格相对该轴的位置是未知的——布片可能跨在轴两侧，
也可能整片挂在一侧，因此测量骨骼本身无法回答"轴要停在离表面多远，网格看
起来才刚好贴合"。它需要买到的只是"末端正好停在面上时不要来回穿越"的余量，
那是一个小的绝对距离，当前取 `0.3 px`，并仍以碰撞体最薄半轴的 `15%` 为上
限，使按躯干定的容差不会吞掉一枚薄饰片。

这个值有上界，而且离得很近。每一像素容差同时也把布料多推离它本该贴着的身体
一像素，接触因此更浅：取 `0.5 px` 时，扫过裙子的腿只驱动了 `0.197 rad`（扫掠
覆盖要求 `0.20`），头部夹具的发丝也停在离脸 `0.04 px` 处再也贴不上去。`0.375`
同样不行，`0.3` 是实测能保住"贴合"的一档。曾试过让接触支撑漏掉一小部分向内
力来补回贴合能力，`2%` 收敛太慢、`10%` 又把风场测试推出界——这是在几个测试之
间来回拟合，已回退：`0.3 px` 本来无需任何补偿即全绿。

曾按受驱动骨自身几何推导（最薄边的一半，再套同一个 `15%` 上限），实测在
`winefox` 与 `zhiban_hanfu` 上**每一对**都由上限胜出，于是末端半径实际只与
碰撞体的最薄半轴成正比、与布片多厚完全无关：同一片裙子贴躯干被顶开
`0.675 px`、贴一枚 `0.3 px` 薄饰片只有 `0.045 px`，越厚的碰撞体把布料悬浮
得越远。布料层不受此影响，它有独立且正确的来源，见上文。
...
同一刚性 reference 下的闭合 Box 会在加载期检查精确拼接面：仅当两面
共面、法向相反且一面被另一面完整覆盖时，才把该接缝从双方可选出口中
移除。部分覆盖、深度重叠和留缝仍保留全部面。这消除了复合躯干内部互相
对抗的法向，又不会把真实外轮廓开洞。

有界 Box/Sphere 另外保存上一帧形状、pivot 和方向。只有上一帧 Top-K
成员在本帧两端都清空、相对行程超过最薄尺度、且端点路径确实穿过保守
包围球时，才在路径中点或至多四个插值姿态上检查接触；命中后提交最深
采样处的合法方向。普通接触和慢速运动不做全局子步。
...
`COMPOUND_SINGLE_BONE` 稳定档，进一步收紧惯性与摆角。
...
命中后写入 `attachmentLeverCorrected`，并使用 `0.12 rad` 安全角和
低转动惯性的固定饰品档。华服 `bone109` 的可见力臂由约
...
- 被持续环境输入压在碰撞体上的部件由 `SpringContactSupport` **托住**，
  而不是只被拦住。位置投影单独解不了恒定需求：输入每帧加进去一段行程，
  投影每帧原样取回，接触保持多久就会颤多久。上一帧碰撞修正的方向就是接触
  法向（投影只能把段沿它撞到的面推开），于是下一帧压向该法向的分量被扣掉，
  等价于支撑面给出的法向力。生产路径已移除静态重力，但移动/动画惯性和风
  仍会形成持续接触。只扣向内分量，斜面照旧滑动、抬离即刻释放。**风也算环境输入**，尽管它以倾角形式到达：
  风偏移的是弹簧被拉向的目标，向内吹时目标落在身体里，投影解不了一个永远
  到不了的目标，布料于是停在拉与推平衡的深度（`0.19 px`）。恢复力不在其中,
  作者姿态连同自带重叠就是平衡点。支撑需连续接触充能（`20%/帧`）而不是首帧
  即满：正在落向布面的部件在抵达那一帧也会被修正，首帧就托住会把它冻结在
  初次接触处，裙子就垂不到身上了；需要回答的是**持续**接触。最后一次修正
  后按 `0.30 s` 衰减——完全被托住的部件恰恰是"已经没有修正可做"的那个，
  当场丢弃支撑就会重启它所消除的颤动。摆角修正不计入：只有碰撞体代表部件
  能停靠的表面。
- 投影修正连续**三帧**反向、**且本帧松弛迭代跑完仍未收敛**时，
  `SpringProjectionDamper` 逐帧把响应降到 `12%`，使矛盾要求收敛到两侧各
  浅浅陷入的折中姿态而不是来回跳；无触发后按 `0.25 s` 恢复。要求连续而非
  单次是必需的：无解的挤压是周期 2 的极限环，每帧都反向；横扫过去的肢体
  每个步幅只反向一次，按单次降响应正好让裙子对被踢失去反应。但只看反向仍
  不够，因为被拉向的目标自己会动——风给段加倾角、阵风方向游走，只是靠在
  身体上的布料反向得和真正无解的一样勤，于是投影只剩 `12%`，风正好把布压
  进它本来只是靠着的身体（`0.19 px`，禁用阻尼即消失）。松弛迭代本身能判：
  单个碰撞体一个 pass 满足后保持满足，要求相反的两个给多少 pass 都不收敛。
- 无投影帧另由 `SpringIntegrationDamper` 处理。实机 `debug.log` 中耳朵和
  头发曾在帧率切换后以 `1.0～1.4 frame` 周期连续反转，且
  `hitF=colF=swgF=0`，证明它不是碰撞环。检测器对作者静息偏移的余弦信号
  保留八帧历史，四次可见反转才进入阻尼；随后将上一物理方向通过最小旋转
  搬运到本帧静息空间，并按 `0.12 s` 释放。这样既能消除 Verlet/动画平衡点
  形成的周期 2，又不会把移动的作者姿态冻结在模型空间。任何摆角、碰撞或
  裙摆锁链投影都会清空该历史，避免与 `SpringProjectionDamper` 重复处理。
- `SpringAuthoredPoseAnchor` 只在实体/动画加速度、转身和风偏置都归零后限制
  朝作者方向的回程；偏移不足 `2 px` 或继续向外运动时完全不介入。超过门槛
  的可见回程上限为 `96 px/s`，因此大网格不会因同一角速度产生几十像素跳变。
  强风若进入被无解接触抑制的代理，则沿风请求到作者方向的短弧做 12 轮二分，
  取满足 `0.01 block` 容差的最近合法点；修正继续进入投影阻尼，而不是另造
  一套振荡状态。
- `SpringInterlockRecovery` 缓存每段到达过的近作者合法方向并随作者参考帧
  搬运。碰撞连续 `0.25 s` 把弹簧回程原样取消，且偏移超过 `55°` 或可见末端
  超过 `4 px` 时，恢复路径可穿过已过时的接触面；速度同时受 `10 rad/s` 和
  `96 px/s` 限制。可见力臂判据覆盖“角度不大、网格却已经移开数十像素”的
  长围裙，普通短段接触仍不会触发。
- 每段除末端外，再对**碰撞体正对着的那个段身位置**投影一次。投影本就
  ...
  生产路径现在把裙片保持在作者画的位置，末端可能位于腿的扫掠行程之外，
  因此段身采样经常是检测踢腿的唯一位置。历史对照中关闭段身采样会把横扫
  响应从 `0.17` 降到 `0.010 rad`；这也是取消静态重力后仍必须保留它的原因。
- 投影结果另按末端线速度 `10 block/s` 限制帧间可见位移，再除以运行时
  骨长换算为每段角预算，使长短骨以相同可见速度解穿，而不是共享固定
  `rad/s`。大幅违反因此在数帧内解开而不是一次瞬移。预算衡量帧间结果而非单帧内部工作量：摆角与碰撞在四轮
...
形状相同的代理只画一次，穿透中的代理始终单独重绘为红色。当前末端对某代理的调整后 `clearance < 0` 时，该代理整组改为
红色，表示穿透超过了自动代理首次标定的静止交叠（显式代理仍以绝对
边界计算），而不是声称整个渲染网格都已精确检测。

线框画在**代理自身的几何**上，不叠加末端半径。这条叠加层要回答的问题是
"碰撞体是否对得上它派生自的那个 cube"，把末端半径加进去会让每个 Box 都
显示成"比模型大了一圈"——包括那些完全正确的：段实际停在再往外一个末端半
径处，而布料层**必须**如此，外层布片按自身半厚被顶开，才能让它的表面而不
是它的骨轴落在下层布面上。因此末端半径也不再参与去重键：它逐段不同，画出
来却是同一个尺寸，否则同一个碰撞体会被重复描边多次。
...
帽具和武器是更强的**刚性装备边界**：TLM 的腰挂、背包和战术 locator
子树直接保持刚性；手部 locator 的父骨另外充当结构锚点，标准 locator
路径以外的直属可见根按手持刚体处理。该结构规则不读取物件名称；命名
装备根再把结论沿 authored 祖先关系传播到匿名后代。因此
`RightHand/MagicStick/Cross` 不会漏入 solver，而空 `flowerBall` 下的
独立捧花花瓣仍可继续评分。帽具内明确的 Hair/Bangs/Fringe 作用域仍按
软体评分；其它歧义帽穗或武器挂件使用 metadata 显式链。
因此薄帽檐和刀光不会再因几何像布片而突破刚性根，metadata 仍保持最高
优先级。
...
- 位于头侧、非包头、从上部悬垂且没有眼嘴/表情后代的 `Mask` 使用保留惯性摆动的 `HEAD_LOCAL RIBBON`；`SHmask/faceplate`、头盔、贴脸面具、带表情子树的面具和手持树保持刚性；
- 新结构角色 `DANGLING_ACCESSORY` 烘焙高参考跟随、高阻尼、低旋转惯性和小四向摆角。它响应实体移动、转身和预设动画的有界瞬态惯性，但不会让刚性主体在静止或匀速头部动画下掉到眼睛前；
...
独立裙片不能共享同一个刚体方向，否则整圈裙子会失去波浪和局部滞后；
但完全独立的逐骨风涡又会把相邻裙片吹成互不相干的布条。当前只对
`SKIRT` 链根建立链间关系，不引入布料自碰撞：

- 构建期按**同一直属刚性安装座**分组；发丝、不同安装座和链内后续段均不配对；
- 每个链根连接到求解顺序中更早、静止末端最近的链根，形成 `n - 1`
  条有向生成树边。leader 在 follower 进入帧循环前已经提交，因此不需要
  第二遍骨架写回，也没有向前修改已传播父子端点的问题；
- 目标距离逐帧取两根链的**当前动画姿态末端距离**，而不是 bind pose 常数。
  允许额外伸长为该距离的 `15%`，并钳在 `0.5～1.5 px`；约束只限制继续
  拉开，不阻止相邻裙片靠拢或保留不同相位；
- 超出允许距离后使用 XPBD：

```text
alphaTilde = 2e-5 / (dt * dt)
deltaLambda =
    (-C - alphaTilde * lambda)
    / (followerInverseMass + alphaTilde)
```

每条边最多三次零分配投影，乘子由 solver 预分配并在每个物理步开始时
清零。follower 质量越大，修正越小；大姿态切换仍复用 `40 rad/s` 的帧间
可见修正预算，不会把合拢的裙缝一次吸过去。

约束优先级是：

```text
integrate
  -> asymmetric swing / collision
  -> skirt branch XPBD
  -> global safety swing / collision
  -> commit state
```

不能在锁链之后再次投影每片裙子的非对称局部锥：围绕身体的相邻片拥有
方向相反的 inward 轴，重新套各自局部锥会立刻撤销刚完成的横向连接。
全局摆幅安全上限与身体碰撞仍最后执行，因此裙摆连贯性不能换来穿体。
Plane、Sphere、Capsule、Box 和摆角继续使用无 compliance 的硬 PBD。
...
- 同一刚性安装座下裙摆链根的有向邻接边与 leader/follower driven slot；
...
- 每个驱动槽的当前/历史方向；
- 每条裙摆邻接边的 XPBD 乘子；每步原地清零，不创建集合或装箱值。
...
XPBD 约束乘子随 solver 构造一次性分配，逐帧只原地清零和复用。
...
- 隐藏网格、locator 与刚性装备不生成本帧自动接触，重新显示后恢复碰撞；
- 生产约束路径静止时回到作者姿态，不因恒定世界重力形成第二平衡点；
- 大动作结束后可见回程有上限，碰撞路径锁不能让部件长期停在作者姿态另一侧；
...
- 网格 Box 自动派生：长头骨的半轴与 cube 精确一致、发丝拿到头部、
  ...
  `winefox_tactics` 的武器/locator/耳机/战术穿戴物和 `sta` 的匿名手持分支
  均不作自动参考、隐藏参考退出本帧窄相且重新显示后惰性恢复、
...
- `winefox` 完整约束解算在 `10/30/50 ms` 交替步长下释放位移和反转阵风，
  但统计时排除一切摆角、碰撞和裙摆锁链投影帧；无接触反转行程保持在
  `0.08 px/frame` 以下，并单独验证静息空间最小旋转搬运保持偏转角与单位长度；
- `AuthoredPoseRecoveryVerification` 让 `winefox` 静止 180 帧，再施加 12 帧
  大冲量并恢复 180 帧：静态最坏偏移不超过 `0.08 px`、冲量确实产生
  `>0.5 px` 位移、最终偏移不超过 `0.20 px`；最新运行样例为
  `0.048 / 130.449 / 0.039 px`；
- 作者锚定单元夹具验证大回程不超过 `96 px/s`、向外运动不受限、风穿透
  二分找到最近合法点；强风四向集成夹具要求相对静止最深穿透增量
  不超过 `0.01 block`；
...
- 同一安装座的三个裙摆链根生成两条有序锁链，不跨安装座或误连发丝；
  强风下相邻片额外张开量低于无锁链对照的 `65%`，同时方向差仍非零；
  内置 `winefox` 分段裙摆与四片式销售员裙摆均能生成锁链；
...
积分周期检测采用点积信号而非 `acos`/开方；同会话开关 A/B 为
`319.7/313.0 µs/frame`，差值方向反直觉且落在噪声内，未发现可分辨回退。

`constraint-layer cost` 是整条约束路径与无约束路径之比，因此包含纯动画
层级、逐段导数预采样、参考空间搬运、摆角投影、姿态驱动以及碰撞。**它不
是碰撞自身的开销**，把它当碰撞成本读会一路指向错误的优化对象。要单看
碰撞，用 `skirt` 的两行对照：`11` 个 Box 摊到约 `+5.3 µs/frame`。
当前约束层为 `7.73x` legacy-active、`0 B/frame`；绝对数字不与旧会话
直接相减。

代理数量不再设上限：能碰到的 cube 全部附着，`winefox` 由 `259` 升到
`3229`。控制成本的是七层结构而不是裁剪精度：

1. **共享形状**。同一 cube 常被几十个段引用，`PreparedCollisionShape`
   按几何去重，每帧只做一次仿射/法线变换；`3229` 个代理背后只有几十
...
3. **组内空间细分**。只按参考骨分组不够：一根骨的 cube 可以铺满整个
   ...
   不超过 `32` 个（加载期一次，帧循环无成本）。细分只决定“测谁”，不
   决定“测出什么”——包围球必须始终覆盖它声称的内容，否则会静默丢掉
   正在接触的碰撞体，游戏里表现为布料穿过身体而 layout 里查不出原因。
4. **Top-K 求解**。末端球一次只可能被少数几个面挡住，而松弛循环每轮重扫
   全集，因此每帧先把最近的 `6` 个送进求解。上一帧活跃项获得
   `1/256 block` 的排序优先量；即使整组本帧已不可达，也会单独复查这些
   活跃项，让相对扫掠能看到两端都清空的穿越。若六席全被身体占满、该段另有
   可达的单向布层代理，则额外保留最近的一块内衬作为第七席，不挤掉身体
   碰撞；无布层节点以及布层本来就在前六的节点都不增加席位。排序每帧按
   实时姿态重做，没有代理被永久丢弃，松弛循环一轮无修正即退出，空槽只花
   一次拒绝。
5. **帧间可达间隙**。有界代理在摆动锥之外时保存正间隙；下一帧先扣除
   pivot、骨长、摆角、轴向、末端尺度和碰撞体绝对模型空间表面运动，余额
   仍为正便跳过重复锥角求解。投影间隙则使用 reference 相对运动，两种界
   不能混用，否则 reference 整体平移会被抵消并漏检。
6. **绑定推迟到排序之后**。绑定要重算碰撞体缩放、末端与段身两份宽限，
   以及由宽限派生的四个半径；排序只用得到可达性判定，一样都不需要。
   因此绑定不再对“扫掠体恰好罩住”的每个代理都做一遍（`winefox` 上每帧
   `685` 次），只对最终留下的那几个做。
7. **间隙留存**（保守推进）。投影为了判断该不该推，本来就得先算出间隙，
...
生产约束路径现在直接省略恒定重力扭矩，不增加逐骨计算。普通作者回程只做
点积与可见距离门控；12 轮二分窄相仅在风目标已经进入被抑制代理的异常分支
执行。当前完整样例为 `322.2 µs/frame`、`0 B/frame`，约束层相对
legacy-active 为 `7.73x`。该数字与旧会话不可直接相减，未做同会话开关
A/B 时不据此声称性能增减。
...
`nanoTime` 自身开销，绝对值虚高，只能看比例）。

加载期静态 BVH 已做过完整原型，并非待办：`winefox` 约慢 `12%`，因为每个
层级节点仍需逐帧变换和分支遍历，成本超过省下的单体球测试，所以回退为
每组最多 `32` 个代理的扁平结构。帧间可达间隙在同会话开关 A/B 中把约束
净开销降低约 `2.5%`，故保留；复用纯动画仿射帧只省不足 `0.4%` 且扩大
跨模块接口，按过度优化回退。
...
- 腿与裙片频率接近时两者同步、相对位移趋近于零，此时没有穿透需要解决
  ...
  幅度”不能用单一步频衡量。
- 摆角与碰撞仍是固定骨长的硬 PBD；XPBD 目前只用于同一刚性安装座下
  的裙摆链根最大距离，不做压缩约束、跨安装座缝合或通用布料自碰撞。
- 同一个 Gecko bone 内所有 cube 共享一次 TRS；占优主簇只改善枢轴/轴和
  安全包络，没有主导簇的分布式复合骨自动保持刚性，也不能让单骨马尾真正弯成多段。真实分段必须由模型提供多个
  不同 pivot 的父子骨；基础 `zhiban bone3/bone9` 因此只做单骨低惯性
...
4. **已完成—刚柔分离**：保守识别固定发饰底座；帽具、武器按刚性装备
   根传播，明确 Hair 作用域与 metadata 柔性后代继续发现；
...
12. **已完成—XPBD 柔性约束**：同安装座裙摆链根构建 O(n) 有向邻接，
    以动画姿态距离为基准限制过度张开，保留松弛、独立起伏和最终碰撞优先级；
13. **已完成—高速与复合表面稳定化**：精确接缝内部面过滤、上一帧
    Top-K 连贯、选择性相对运动扫掠、帧间可达间隙和按骨长换算的线性修正预算；
14. **已完成—作者姿态锚定**：生产路径取消静态重力二次下垂，无驱动回程按
    可见像素限速，缓存近作者合法目标以解除长网格碰撞锁，并对风压入被抑制
    代理的情况搜索最近作者侧合法边界。
...
- [Sable physics source](https://github.com/ryanhcode/sable)
- [Create Aeronautics / Simulated source](https://github.com/Creators-of-Aeronautics/Simulated-Project)
