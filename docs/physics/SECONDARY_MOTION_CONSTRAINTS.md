...
重力按 `(0, -g, 0)` 恒定注入。恒力作用在弹簧上只有一个结果：停在"力与
恢复力平衡"处，而不是停在被画出来的位置。作者画裙摆下垂、腰带侧躺时，
画的**已经是**重力作用后的形态，再叠加一次世界下方的恒力等于把重力算了
两遍——上翘的发丝会略垮、横向的丝带会略歪，且调小 `gravityScale` 只能
减轻不能消除，因为位移的存在本身就是恒力的性质。

这点位移是**被接受的代价**，因为它同时是垂坠感的来源。曾按静止方向
`rest` 分解重力来消除它：
...
平行分量只改变"被拉回去的力度"而不改变"回到哪里"（方向随后被归一化），
垂直分量按 `displaced` 淡入，因此静止时严格保持作者姿态，被吹开或踢开后
仍朝真实下方坠落。竖直下垂的骨与世界重力共线，两种算法结果相同，差异只
出现在横向、上举和斜向部件上。该方案达到了它的目标——静止零位移——但游戏
内观感是部件不再下沉、显得发硬，因此退回世界重力。

需要"零静态垂量"的类型不靠改重力方向解决，而是把 `gravityScale` 设为零：
`HEAD_SHELL` 与自动悬垂饰品即如此，见上一节。
...
- 被恒力压在碰撞体上的部件由 `SpringContactSupport` **托住**，而不是只被
  拦住。位置投影单独解不了这件事：重力每帧加进去一帧的行程，投影每帧原样
  取回来，姿态保持多久就颤多久，而且这不是小幅噪声——`winefox` 站着不动
  三秒，最抖的一段走了 `1.53 rad`。上一帧碰撞修正的方向就是接触法向（投影
  只能把段沿它撞到的面推开），于是下一帧的外力里压向该法向的分量被扣掉，
  等价于支撑面给出的法向力，部件在自己完全承载的载荷下静止。只扣向内分量，
  斜面照旧下滑、抬离即刻释放。**风也算环境输入**，尽管它以倾角形式到达：
...
- 每段除末端外，再对**碰撞体正对着的那个段身位置**投影一次。投影本就
  ...
  段身采样的权重随重力算法变化。世界重力下裙片被往外下拽，末端也能进入
  腿的行程，因此末端与段身都有贡献：`winefox` 上关掉段身采样，横扫响应
  由 `0.35` 掉到 `0.094 rad`。而在相对姿态重力下裙片挂在作者画的位置、
  末端落在腿的行程之上从头到尾碰不到腿，段身采样一度是**唯一**的检测手段
  （`0.17` 掉到 `0.010 rad`）。两种算法下它都是必需的。
...
iterative-active-legacy: 34067.4 ns/frame, nodes=153/181, allocation=0.00 B/frame
iterative-active-constrained: 284930.7 ns/frame, nodes=153/181, allocation=0.00 B/frame
legacy-equivalent solver speedup: 0.96x
constraint-layer cost: 8.36x legacy-active
...
重力曾改为相对姿态求解（每段多一个点积，几乎不花钱），后因观感发硬退回
世界重力。同一次会话内做过对照：`298 µs` 对 `324 µs`，方向与直觉相反且
落在噪声里。同一份代码在不同会话测出过 `298` 与 `350 µs`，因此这个量级
的差异只能同会话对照，跨会话数字不可比。
