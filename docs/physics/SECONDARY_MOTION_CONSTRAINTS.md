...
> 状态：阶段 1～4 已落地；身体代理与按需 XPBD 柔性约束尚未实施。
>
...
和 layout 静态约束数据实现：参考骨旋转增量先搬运历史状态，积分后将
方向投影到四向摆角椭圆，再执行局部 Backstop 与头部球体投影，并把
合法方向写回当前和历史状态。单纯调整 `STIFFNESS`、`GRAVITY_POWER`
或 `MAX_ANGLE` 仍不能替代这些硬约束。
...
4. 恢复骨长；
5. 投影动画姿态相对的非对称摆角约束；
6. 投影当前已实现的 Backstop 与头部球体碰撞；
7. 最多交替复查摆角和碰撞四轮；胶囊留待阶段 5；
8. 将最终合法方向写回当前和历史状态；
...
当前已由静止 `Head` AABB 自动拟合头部球体，并从驱动骨几何最小
横截面估计 `hitRadius`。碰撞连接点使用 `BoneKinematics` 校正后的
...
两个端点评分仍接近时不移动 authored pivot，并降低该骨骼摆角。最终连接点通过匹配
Gecko 渲染顺序的绑定姿态仿射矩阵变换到碰撞空间。投影直接解固定
骨长下的最小点积边界，不会先移动端点再以归一化破坏碰撞结果。头部短胶囊、躯干和腿部代理
仍属于阶段 5。附着轴极性与 pivot 是否被替换分别判断，避免共享或
重叠 pivot 的续段仍沿反方向运动。自动碰撞只分配给没有受驱动父骨的链根/短发饰，
避免用绑定姿态枢轴错误约束已经被柔性父段移动的后代；后代仍继承
角约束，待运行时端点层级传递实现后再启用逐段碰撞。
...
运动学连接点；只拒绝该底座，不排除其柔性后代。回归夹具覆盖
`winefox_elf/flower`、`rice_cake_fox/Balls`，并继续要求
`LeftPony`、`RightPony`、纸板狐 `bone34` 等柔性发型保持驱动。
...
当前 Backstop、球体和摆角均为无 compliance 的硬 PBD 投影。
...
XPBD 约束乘子与身体代理数组尚未分配；它们只会随对应阶段按需加入。
...
当前 Windows/JDK 17、`winefox`、20,000 测量帧样例：
...
5. **待实施—身体代理**：扩展到裙摆、披风和胸前饰件；
6. **按需实施—XPBD 柔性约束**：只在出现链间柔性关系需求时加入。
