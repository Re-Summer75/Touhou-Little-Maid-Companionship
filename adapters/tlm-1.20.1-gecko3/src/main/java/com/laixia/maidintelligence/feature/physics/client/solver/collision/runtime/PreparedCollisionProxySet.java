...
public final class PreparedCollisionProxySet {
    ...
    /** Live slot of the collider that most recently moved this segment. */
    private int lastResponder = -1;
     ...
     */
    public boolean suppressCompetingResponders(
            Vector3f projectedDirection,
            CollisionScratch scratch
    ) {
        int count = liveCount();
        if (lastResponder < 0 || lastResponder >= count) {
            return false;
        }
        scratch.setMeshExtent(meshHalf, meshAxisX, meshAxisY, meshAxisZ);
        boolean suppressed = false;
        for (int slot = 0; slot < count; slot++) {
            if (slot == lastResponder) {
                continue;
            }
    ...
    private void releaseSuppressed(
        ...
        for (int slot = 0; slot < count; slot++) {
            liveProxy(slot).holdSuppression(direction, scratch);
        }
    }
