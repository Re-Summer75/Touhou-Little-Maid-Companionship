...
final class SpringProjectionDamper {
    ...
    static boolean apply(
        ...
        if (!collisionCorrected
            ...
            long reentries = recordSlow(
                    state.projectionReentries[drivenSlot],
                    false
            );
            state.projectionReentries[drivenSlot] = reentries;
            state.projectionCorrectionGap[drivenSlot] = Math.min(
                    SLOW_WINDOW,
                    state.projectionCorrectionGap[drivenSlot] + 1
            );
        ...
        boolean reentered = resumedAfterGap(
                state.projectionCorrectionGap[drivenSlot]
        ) && significantReentry(cx * cx + cy * cy + cz * cz);
        state.projectionCorrectionGap[drivenSlot] = 0;
        long reentries = recordSlow(
                state.projectionReentries[drivenSlot],
                reentered
        );
    ...
    static void competitorsSuppressed(int drivenSlot, SpringBoneState state) {
        state.projectionCorrections[drivenSlot].zero();
        state.projectionReversals[drivenSlot] = 0;
        state.projectionReentries[drivenSlot] = 0L;
        state.projectionCorrectionGap[drivenSlot] = 0;
        state.projectionQuiet[drivenSlot] = 0;
        state.projectionDamping[drivenSlot] = 0.0F;
    }
