...
public final class DefaultToolDurabilityPolicy {
    public static final DefaultToolDurabilityPolicy INSTANCE = new DefaultToolDurabilityPolicy();
    ...
    public int warningThreshold(int maxDamage) {
        int tenPercentRoundedUp = (Math.max(0, maxDamage) + WARNING_PERCENT_DIVISOR - 1)
    ...
    public boolean isLowDurability(int maxDamage, int damageValue) {
        return maxDamage > 0
                && remainingDurability(maxDamage, damageValue) <= warningThreshold(maxDamage);
    }
}
