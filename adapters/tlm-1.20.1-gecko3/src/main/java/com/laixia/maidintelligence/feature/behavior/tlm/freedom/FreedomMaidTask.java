...
public final class FreedomMaidTask implements IRangedAttackTask {
    ...
    /**
     * Wandering is a decision, and decisions are hers here.
     *
     * <p>This returned true, on the reasoning that idling visibly beats
     * standing still between decisions. It does — but the host's wandering is
     * not idling, it is a second opinion about where she should be, written at
     * {@code NATIVE_SOFT} authority every time it fires. Measured in the combat
     * timeline: a maid backing away from three vindicators, losing her walk
     * target to {@code RANDOM_STROLL}, and standing still inside a correct
     * decision to leave.
     *
     * <p>Free mode no longer registers it at all ({@link FreedomBrain}), so
     * this switch decides nothing. It is stated anyway, because a switch that
     * says "yes" while the thing it governs is gone reads as an intention, and
     * the next person to restore a behaviour would restore this one too.
     */
    @Override
    ...
    /**
     * Fleeing is a decision too, and this mod already makes it.
     *
     * <p>Panic is not a mood in the host, it is a Brain activity that seizes
     * control: {@code MaidPanicTask} fires on {@code isHurt || hasHostile} —
     * a hostile merely being nearby is enough — and on activation it erases
     * {@code WALK_TARGET} and {@code LOOK_TARGET}. Returning a constant true
     * therefore had the combat action write a movement target every tick and
     * panic delete it every tick, and an armed maid facing a zombie did
     * nothing at all.
     *
     * <p>The fix at the time was to answer from her inventory, which made the
     * two exclusive rather than simultaneous. That was still two systems
     * agreeing to take turns. The combat intent fires on hostile pressure
     * without asking whether she is armed, and an unarmed maid is exactly the
     * case {@code EngagementRiskPolicy} answers {@code WITHDRAW} for — so
     * running away was already hers, and the host's copy of it only ever
     * competed.
     *
     * <p>Now false outright, and free mode does not register the activity
     * either. An unarmed maid still runs; she runs because she decided to.
     */
    @Override
