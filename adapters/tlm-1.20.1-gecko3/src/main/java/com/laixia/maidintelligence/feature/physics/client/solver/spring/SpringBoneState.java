...
final class SpringBoneState {
    ...
    /**
     * Frames on which projection resumed after a quiet gap, newest in the low
     * bit. A single-surface limit cycle does not reverse its corrections: the
     * collider ejects in the same direction each time while the spring silently
     * pulls back between ejections, so reversal history cannot see it.
     */
    final long[] projectionReentries;
    /** Consecutive frames with no significant projection correction. */
    ...
    /** Bit 1 for a swing limit, bit 2 for a collider, per driven slot. */
    public final int[] lastProjectionSource;
    /**
     * The pose the spring pulls each segment back towards, kept per slot.
     *
     * <p>Not the bind pose: an animation that rotates a driven bone itself — a
     * seated skirt is keyframed to sixty-five degrees — moves the equilibrium with
     * it. This is that equilibrium, which is what a diagnostic asking how far
     * physics has displaced a part needs to measure from. The scratch holds it
     * during a solve but is reused by the next segment.
     */
    ...
    /** Consecutive frames with no collider against the segment, counted down. */
    final int[] contactHold;
    ...
    SpringBoneState(PhysicsSolverLayout layout) {
        ...
        restDirections = new Vector3f[drivenCount];
